...
if (isset($user)) {
	...
	$newNoticeQuantity = Notice::find()
		...
		->count();
	User::$topuser['user/notice']['quantity'] = $newNoticeQuantity;

	$newOrderQuantity = Order::find()
		->where(['user_id' => $user->id, 'status' => Order::STATUS_NEW])
		->count();
	User::$topuser['order/list']['quantity'] = $newOrderQuantity;

	$newMessageQuantity = Message::find()
		->where(['recipient_id' => $user->id, 'read' => '0'])
		->count();
	User::$topuser['message/list']['quantity'] = $newMessageQuantity;
}
...
?>
	...
	<ul class="menuTop__tabs">
		...
		<?php foreach (User::$topuser as $key => $value): ?>
