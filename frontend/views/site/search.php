...
<form action="" method="POST" id="js_saerchpage_form_dishtype_id">
	<input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
	<input type="hidden" name="form_dishtype_id" value="1" />
						...
						<div class="categories">
							...
							<div class="categories__inner">
								<div class="categories__list js-moveSliderCategoriesContainer">


<?php foreach ($dishtypes as $key => $dt): ?>
	<label class="categories__item">
		
			<input name="dishtype_id[]" type="checkbox" class="categories__checkbox js_saerchpage_inputDishTypeID" value="<?= $dt->id ?>" style="visibility: hidden; position: absolute;"<?= ( in_array($dt->id, explode(',',$bindedValues['dishtype_id'])) )?' checked':'' ?>>
			
			<div class="categories__bg"><?= $dt->header ?></div>
			<div class="categories__link"><?= $dt->header ?></div>
...
<!-- <li class="categories__item">
	<a class="categories__link categories__link_4" href="#">header</a>
							...
							<div class="categories__count">Выбрано: 6</div>
						</div>
</form>
						...
						<div class="menuProfile">
							<div class="preLinksBox js-toggleBoxAddBg js-toggleBoxAddBg__toggle">
								<span class="preLinksBox__text">Категории</span>
							</div>
							<div class="linksBox linksBox_bb linksBox_p10-30-0 linksBox_start js-toggleBoxAddBg__box js-showAll__box">
								<div class="linksBox__wrap js-showAll__innerBox">
...
<form action="" method="POST" id="js_saerchpage_form_kitchen_ids">
	...
	<?php foreach ($kitchens as $key => $k): ?>
		<label class="linksBox__item">
			<input name="kitchen_ids[]" type="checkbox" value="<?= $k->id ?>" class="linksBox__checkbox js_saerchpage_input_kitchen_ids" style="visibility: hidden; position: absolute;"<?= ( in_array($k->id, explode(',',$bindedValues['kitchen_ids'])) )?' checked':'' ?> />
			<div class="linksBox__bg"><?= $k->header ?></div>
			<div class="linksBox__link linksBox__link_checked"><?= $k->header ?></div>
		</label>
	<?php endforeach ?>
</form>
...
<?php endif ?>
								...
								<div class="linksBox__link js-showAll js-showAll__toggle">Посмотреть все...</div>
						...
						<form action="" method="post" id="js_saerchpage_form_wide" class="wideSearch wideSearch_db wideSearch_h65 wideSearch_grey js-toggleBlog__parent">
							<input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
							<div class="wideSearch__header">
								...
								<div class="wideSearch__rightBox">
									<input name="searchquery" class="g-input g-input_500px g-input_search g-input_wh" type="search" placeholder="Поиск по поварам ..." value="<?= $bindedValues['searchquery'] ?>" />
						...
						<div class="map">
							<div class="map__topContainer map__topContainer_h45vw js-toggleToHeight__box">
								<form class="map__form" action="" method="post">
									<input type="hidden" name="_csrf" value="<?=Yii::$app->request->getCsrfToken()?>" />
									<div class="slidesBox slidesBox_map">
										<div class="slidesBox__header">
											...
											<div class="slidesBox__wrap">
												...
												<input name="price_from" value="<?= $bindedValues['price_from'] ?>" class="slidesBox__val slidesBox__val_orange" type="text" id="amountMin" />
												<div class="slidesBox__separator">-</div>
												<input name="price_to" value="<?= $bindedValues['price_to'] ?>" class="slidesBox__val slidesBox__val_orange" type="text" id="amountMax" />
									...
									<!-- <input type="submit" name="submitform" value="submit" style="visibility: hidden; position: absolute;" /> -->
								</form>
